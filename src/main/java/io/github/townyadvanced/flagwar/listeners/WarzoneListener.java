...
public class WarzoneListener implements Listener {
     ...
     * expression {@link FlagWarConfig#isEditableMaterialInWarZone(Material)} is true.
     *
     ...
     * being used is in the allowed items list in the WarZone section of the FlagWarConfig.
     *
     ...
     * of switches is enabled in the WarZone section of the FlagWarConfig.
     *
     ...
     * is under attack, and the FlagWarConfig is configured to allow fires in WarZones.
     *
     ...
     * corresponds to a Cell which is under attack, and the FlagWarConfig is configured to allow those
    ...
    public void onExplosionDamagingBlocks(final TownyExplodingBlocksEvent event) {
        if (!FlagWarConfig.isAllowingAttacks()
        || !FlagWarConfig.explosionsBreakBlocksInWarZone()) {
            return;
        }
